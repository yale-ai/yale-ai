    ...
    <div className="bg-stage min-h-dvh text-white">
      ...
      <div className="relative z-10 mx-auto flex max-w-6xl flex-col px-5 py-5 md:px-8 md:py-6">
        ...
        <main className="pt-10 md:pt-16">
          ...
          <p className="mt-5 max-w-2xl text-[1rem] leading-relaxed text-white/75">
          ...
          <section className="card mt-12 p-6 md:p-8">
            ...
            <p className="mt-3 max-w-prose text-[0.98rem] leading-relaxed text-white/75">
              We are looking for people to own marketing, workshops, and project teams this year. If that is you, write
              to <Link href={`mailto:${CONTACT_EMAIL}`} className="u-lime text-white">{CONTACT_EMAIL}</Link>.
            </p>
          </section>
        </main>
        <Footer />
  ...
  return (
    <li className="card card-hover flex h-full flex-col overflow-hidden">
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-black/40">
        {m.photo ? (
          ...
          <div className="grid h-full w-full place-items-center bg-[radial-gradient(circle_at_30%_20%,rgba(79,216,200,0.28),transparent_55%),radial-gradient(circle_at_80%_90%,rgba(217,228,168,0.22),transparent_55%)]">
            <span className="font-pixel text-[3.6rem] font-medium text-white/90">{initials(m.name)}</span>
          </div>
        )}
      ...
      <div className="flex flex-1 flex-col gap-2 p-5">
        ...
        {meta && <p className="font-mono text-[0.72rem] text-teal">{meta}</p>}
        {m.bio && <p className="text-[0.86rem] leading-relaxed text-white/65">{m.bio}</p>}
        <div className="mt-auto flex flex-wrap items-center gap-1.5 pt-3">
          ...
          <span className="ml-auto truncate text-[0.72rem] text-white/40">{m.email}</span>
        </div>
      </div>
    </li>
  );
...
function IconLink({ href, label, external, children }: { href: string; label: string; external?: boolean; children: React.ReactNode }) {
  return (
    <Link
      ...
      className="grid h-8 w-8 place-items-center rounded-full border border-white/15 bg-white/[0.04] text-white/65 transition-colors hover:border-lime/60 hover:text-white"
    >
      {children}
    </Link>
  );
